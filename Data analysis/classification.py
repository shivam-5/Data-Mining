...
def decision_tree_error(X_train, y_train, X_test, y_test):
    ...
    # setting max-depth to 5 to avoid overfitting
    # keeping all other parameters default
    classifier = tree.DecisionTreeClassifier(max_depth=5, random_state=41)
...
sample_set = non_missing_instances.sample(n=missing_instances.shape[0], replace=False, random_state=41)
...
d_dash_2 = d_dash.fillna(d_dash.mode().iloc[0])
# Encode the attributes of training and test data with LabelEncoder
